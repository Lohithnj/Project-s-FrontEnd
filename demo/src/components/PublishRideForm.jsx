...
function PublishRideForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const rideId = res.data; 
      console.log("🎯 rideId received from backend:", rideId);
      if (res.status === 200) {
       ...
       // sessionStorage.setItem("rideId", res.data.rideId);
        //const rideId = res.data.rideId; // Ensure your backend returns it
        sessionStorage.setItem("rideId", rideId); // ✅ Store it for next step
        navigate("/route");
      } else {
